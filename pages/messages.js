...
function Messages({chatsData, user}) {
    ...
    // persistencia de estado para la query url
    ...
    useEffect(() => {
        ...
        if (socket.current) {
            /* PRUEBA DE CONEXION A SOCKET
            socket.current.emit("hello", {name: "alan", age: "27"});
    ...
    useEffect(() => {
        if (socket.current) {
            ...
            socket.current.on('newMsgReceived', async ({newMsg}) => {
                ...
                // cuando el chat esta abierto en el navegador
